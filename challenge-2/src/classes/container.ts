...
export class Container {
   ...
   *
   * @param product A new object
   * @returns The object id
   */
  async save(product: IProduct): Promise<number> {
    try {
      const products: IProducts[] = fs.existsSync(this.filePath)
        ? this.getAll()
        : [];
      const id = products.length + 1;

      if (products) products.push({ ...product, id });
